...
class PatchedLightEffect(PropertyGroup):
    type = EnumProperty(
        name="Effect Type",
        description=(
            "Type of the light effect: color ramp-based, image-based, vertex"
            " color driven or custom function"
        ),
        items=[
        ...
        *,
        ...
        def apply_single_mode(*, clamp_temporal_to_duration: bool = False) -> None:
            ...
            if self.type == "VERTEX_COLOR":
                ...
                if cache_entries:
                    vertex_colors = [None] * num_positions
                    for entry in cache_entries:
                        drone_index = entry.get("drone_index")
                        color = entry.get("color")
                        if (
            ...
            outputs_x_arr = np.clip(outputs_x_arr + offset_x, 0.0, 1.0)
            valid_x_mask = np.isfinite(outputs_x_arr)

            if outputs_y_arr is not None:
                outputs_y_arr = np.clip(outputs_y_arr + offset_y, 0.0, 1.0)
...
def patch_light_effect_collection():
    ...
    if getattr(LightEffectCollection, "_original_append_new_entry", None) is None:
        LightEffectCollection._original_append_new_entry = getattr(
            LightEffectCollection, "append_new_entry", None
        )

    if getattr(LightEffectCollection, "_original_append_new_entry", None) is not None:
        LightEffectCollection.append_new_entry = _append_new_entry
...
def unpatch_light_effect_collection():
    ...
    original = getattr(LightEffectCollection, "_original_append_new_entry", None)
    if original is not None:
        LightEffectCollection.append_new_entry = original
        LightEffectCollection._original_append_new_entry = None
        ...
        if entry is not None:
            ...
            if entry.texture:
                if entry.type == "COLOR_RAMP":
                    ...
                    box = layout.box()
                    box.label(text="Vertex colors will drive drone colors")
            ...
            if (
                entry.type in {"COLOR_RAMP", "IMAGE"}
                and output_type_supports_mapping_mode(entry.output)
            ):
